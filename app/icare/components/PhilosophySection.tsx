...
export const PhilosophySection: React.FC<PhilosophySectionProps> = ({ lang, onNavigate }) => {
  ...
  // ContentProvider layer — BE serves Unsplash default + EN/AR values.
  const { val: philosophyHeadlineCp } = useContent('home.philosophy.headline', { lang, fallback: '' });
  const { val: philosophyTextCp } = useContent('home.philosophy.text', { lang, fallback: '' });
  const { val: philosophyCtaCp } = useContent('home.philosophy.cta', { lang, fallback: '' });
  const { val: philosophyImageCp } = useContent('home.philosophy.image', { lang, fallback: '' });
  return (
...
viewport={{ once: false }}
        ...
        >
          <ImageWithFallback
            src={philosophyImageCp || philosophyImage}
      ...
      <div className="icare-full-banner__copy">
        ...
        >
          {philosophyHeadlineCp || philosophyHeadline}
        </motion.h2>
        ...
        >
          {philosophyTextCp || philosophyText}
        </motion.p>

        <motion.button
          onClick={() => onNavigate('story')}
          aria-label={philosophyCtaCp || philosophyCta}
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
