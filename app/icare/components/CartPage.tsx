    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-6 max-w-md">
          <div className="w-24 h-24 mx-auto rounded-full bg-[#F2F1ED] flex items-center justify-center">
            <ShoppingBag size={40} className="text-[#CCC]" />
          </div>
          <h1 className="text-[28px] md:text-[36px] font-black lowercase tracking-tight text-[#333]">{text.empty}</h1>
          <p className="text-[14px] text-[#888]">{text.emptyDesc}</p>
          <button onClick={() => navigateToPage('shop')} className="px-10 py-4 bg-black text-white rounded-full text-[12px] font-black uppercase tracking-[0.2em] hover:bg-black/90 transition-colors">
            {text.shopNow}
          </button>
        </motion.div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      <div className="max-w-[1200px] mx-auto px-4 md:px-8 pt-8">
        <button onClick={() => navigateToPage('shop')} className="inline-flex items-center gap-2 text-[13px] text-[#666] hover:text-black mb-8 transition-colors">
          <ArrowLeft size={16} /> {text.continue}
        </button>

        <h1 className="text-[32px] md:text-[42px] font-black lowercase tracking-tight text-[#222] mb-10">{text.title} ({cartCount} {text.items})</h1>

        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-6">
            {cartItems.map((item) => (
              <motion.div key={item.id} layout className="flex gap-4 md:gap-6 p-4 rounded-[16px] border border-black/5 hover:border-black/10 transition-colors">
                <div className="w-24 h-24 md:w-28 md:h-28 bg-[#F8F7F4] rounded-[12px] flex items-center justify-center p-3 flex-shrink-0">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-4 mt-3">
                    <div className="flex items-center gap-2 bg-[#F2F1ED] rounded-full">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-[#666] hover:text-black transition-colors"><Minus size={14} /></button>
                      <span className="text-[12px] font-bold min-w-[20px] text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center text-[#666] hover:text-black transition-colors"><Plus size={14} /></button>
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#F2F1ED] p-6 md:p-8 rounded-[20px] sticky top-28">
