...
export const CartDrawer = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  // ContentProvider overrides — 5 cart keys.
  const { val: cartBagLabelCp } = useContent('cart.bag.label', { lang, fallback: '' });
  const { val: cartEmptyDrawerCp } = useContent('cart.empty.drawer', { lang, fallback: '' });
  const { val: cartContinueShoppingCp } = useContent('cart.continue.shopping', { lang, fallback: '' });
  const { val: cartShippingDisclaimerCp } = useContent('cart.shipping.disclaimer', { lang, fallback: '' });
  const { val: cartCheckoutLabelCp } = useContent('cart.checkout.label', { lang, fallback: '' });
        ...
        <>
          ...
          >
            <div className="flex justify-between items-center mb-12">
              <h2 className="text-[12px] tracking-[0.2em] uppercase font-medium">{cartBagLabelCp || cartBagLabel} (0)</h2>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <p className="text-[11px] tracking-wider text-[#84827E] uppercase mb-8">{cartEmptyDrawerCp || cartEmptyDrawer}</p>
              <button onClick={onClose} className="bg-[#67645E] text-white w-full min-h-[44px] py-4 rounded-full text-[10px] tracking-[0.2em] uppercase font-medium hover:bg-[#5A5853] transition-colors">
                {cartContinueShoppingCp || cartContinueShopping}
              </button>
            </div>
            ...
            <div className="pt-8 border-t border-[#DDDDDD]">
              ...
              <p className="text-[9px] text-[#84827E] tracking-wider uppercase mb-6 text-center">{cartShippingDisclaimerCp || cartShippingDisclaimer}</p>
              <button disabled className="bg-[#67645E]/10 text-[#67645E]/40 w-full min-h-[44px] py-4 rounded-full text-[10px] tracking-[0.2em] uppercase font-medium cursor-not-allowed">
                {cartCheckoutLabelCp || cartCheckoutLabel}
              </button>
            </div>
          </motion.div>
        </>
